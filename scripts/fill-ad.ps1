...
Import-CSV -delimiter ";" c:\vagrant\scripts\users.csv | foreach { New-ADUser -SamAccountName $_.SamAccountName -GivenName $_.GivenName -Surname $_.Surname -Name $_.Name -Path "OU=Users,OU=HeadQuarter,OU=Locations,DC=windomain,DC=local" -AccountPassword (ConvertTo-SecureString -AsPlainText $_.Password -Force) -Enabled $true }
